...
void newtile()
{
	...
	// 给新方块赋上颜色（随机）
	vec4 randColors[5] = { orange, green, red, blue, yellow };
	std::uniform_int_distribution<int> di(0, 4);
	int randColorIndex = di(DRE);
	curTileColor = randColors[randColorIndex];
	vec4 newcolours[24];
	for (int i = 0; i < 24; i++) {
		newcolours[i] = curTileColor;
	}
